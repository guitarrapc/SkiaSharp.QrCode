...
/// <summary>
/// One encoding-mode run of a planned Standard QR bit stream: a contiguous slice of
/// the source text plus the value its character count indicator carries. Packed small
/// and reference-free on purpose — plans live in a caller-lent <see cref="Span{T}"/>
/// (stack for short content, pooled for long), so a run must never own storage.
/// </summary>
internal readonly struct QRSegment
{
    ...
    public QRSegment(int modeIndex, int start, int length, int unitCount)
    {
        ...
        // plannable content (QRSegmentPlanner.MaxPlannableChars) stays far below
        // ushort.MaxValue.
