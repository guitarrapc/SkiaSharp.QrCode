...
Capacity pricing guard: Byte mode costs `8 × length` bits, so a data length past `int.MaxValue / 8` wraps negative and reads as a fit. All three selectors reject above `CapacityGuard.MaxPriceableDataLength` before pricing. The rejection is exact rather than conservative — the largest symbol of any symbology holds 7,089 units — and it is one compare on the fit path. Reachable only with a quarter-gigabyte input, so `CapacityOverflowGuardTest` drives the selectors directly instead of allocating one.
