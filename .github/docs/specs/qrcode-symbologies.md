...
Capacity pricing overflow: Byte mode costs `8 × length` bits, so a data length past `int.MaxValue / 8` wraps negative and reads as a fit. Standard QR and Micro QR price in `long`; rMQR keeps its `int` arithmetic (asserted against exact bit counts elsewhere) and instead rejects lengths it cannot price, which is safe there because every caller validates its arguments before reaching `Fits`. The difference is deliberate: a length fast-path is only correct where nothing downstream of it still needs to reject an argument, and putting one ahead of the Micro QR version check both swallowed the argument error and handed an unvalidated version to the table-indexing error builder. Reachable only with a quarter-gigabyte input, so `CapacityOverflowGuardTest` drives the selectors directly instead of allocating one.
