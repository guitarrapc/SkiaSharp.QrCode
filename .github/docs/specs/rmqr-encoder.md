...
- *Trivial bound.* One O(n) pass and no table: each character at the cheapest rate any mode could give it (10 bits per 3 digits, 11 per 2 alphanumerics, 8 per byte), plus one minimal header. A partial group only ever costs more per character, so summing these rates cannot exceed a real plan. When no better-ranked version holds even this, no split can move the symbol and nothing further runs — which is what makes `Optimal` free on content it cannot help.
- *Floor.* A cost run at the narrowest count indicator widths any version uses (4 / 3 / 3), deferred until a candidate clears the trivial bound. Widening a count indicator only raises the price of the run carrying it, and the minimum over plans of a pointwise larger cost is itself larger, so this is a lower bound everywhere.
