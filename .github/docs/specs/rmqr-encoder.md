...
**How the optimum is exact.** A run does not cost a constant per character (Numeric packs 3 digits into 10 bits, Alphanumeric 2 characters into 11), so the dynamic program carries the group remainder in its state rather than rounding a per-character average, which would misprice the tail of every run. The state layout and transitions are in `ModeSegmenter`, shared with the Standard QR planner since the two symbologies differ only in header widths (3-bit vs 4-bit mode indicator, their own count indicator tables) — a single implementation keeps the two cost models from drifting apart, which mattered once each carried its own copy of the UTF-8 surrogate cost rules. The version scan, its bounds and the selector integration stay in `RmQRSegmentPlanner`: they depend on rMQR's version space, not on the cost model.
