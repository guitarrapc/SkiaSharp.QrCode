...
**Bracketing the scan.** The scan is best-first, i.e. smallest-version-first, so for mixed content most early ranks are hopeless. A single cost run at the narrowest count indicator widths any version uses (4 / 3 / 3) brackets every other version from both sides, so most candidates need no run of their own.

- *Floor.* Widening a count indicator only raises the price of the run carrying it, and the minimum over plans of a pointwise larger cost is itself larger, so that run's cost is a lower bound everywhere. Below it a version cannot fit.
- *Ceiling.* The same run also yields a plan, and re-pricing that plan at a version costs the floor plus one count indicator delta per run it contains — arithmetic, no second cost run. Being an actual plan, its price bounds the optimum from above, so a version holding it is known to fit.

Only versions between the two bounds are priced for real. Against the Single encode of the same content in the same benchmark run, the floor alone took the 150-byte worst case from 13.2x to 3.0x, and the ceiling cut a further 26-42%, most where the scan hurt most.
...
**What it costs, and what drives it.** Planning allocates nothing. Its cost is driven by *how much the split helps*, not by how mixed the content looks: the lower a split drives the bit cost, the more candidate versions clear the floor and get priced. `RmQRSegmentationEncode` pins this by varying content shape with a same-run Single partner on every row, so its Ratio column is the multiplier directly. Sorting that column separates the two populations exactly: content the split loses on (alternating every character 2.3x, 150 lowercase 2.3x, all alphanumeric 3.4x) sits below content the split wins on (60 lowercase + 60 digits 4.7x, 150 half and half 5.4x, alternating in tens 6.2x), with all-numeric short-circuited at 1.0x. Cost is linear in length at a fixed shape (20 / 60 / 120 / 150 characters of half letters half digits: 1.1 / 2.9 / 4.3 / 5.6 us). The consequence worth stating plainly is that the expensive inputs are the rewarding ones.
