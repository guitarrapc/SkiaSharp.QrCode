...
- **Transitive pinning is a per-project property, and "on for apps, off for packages" is the shape that keeps both benefits.** Pinning exists to hold applications' transitive versions to the central file; on a packable project it rewrites the nuspec instead. Nothing a consumer resolves changed when it was switched off for the three packages, which is the evidence that the promotion was never a dependency, only a declaration.
