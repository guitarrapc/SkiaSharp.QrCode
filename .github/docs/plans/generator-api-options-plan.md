...
- **A review found a doc/behaviour mismatch that had shipped in 1.1.1 and that this phase preserved by copying.** The obsolete `QRCodeGenerator.GetRequiredBufferSize` documented `ArgumentOutOfRangeException` for content that exceeds Version 40; the measured throw is `InvalidOperationException`, raised by the internal `GetVersion`. Docs corrected, behaviour deliberately not, since changing what a released overload throws is exactly the break the deprecation window exists to avoid. Probing also surfaced that **Standard QR and Micro QR disagree on this**: Micro QR reports the same overflow as `ArgumentException`. `ObsoleteSizing_KeepsItsReleasedExceptions` now pins both, which is what makes the migration document's "same signatures, same exceptions, same messages" a checked claim rather than a promise. The reviewer also claimed the same defect on the `Try` overload; measurement says otherwise — `Try` does not throw for overflow at all — so only the incompleteness there (an undefined ECC level raises `ArgumentException`) was worth documenting.
- **Three review rounds all reported the same defect, which is the signal to stop patching lines and go measure.** Every comment was an XML `<exception>` tag that disagreed with what the code raises. Patching them one at a time would have kept the loop running, so the whole sizing surface was probed instead: 30 argument-error cases across the five public sizing methods, exception type recorded for each, docs rewritten from that table, and the table itself pinned in `SizingExceptionContractTest`. **A doc comment is the only part of a public API with no checker behind it**, which is why it drifted in the first place and why the fix had to be a test rather than better proofreading. Probing also surfaced two released inconsistencies that are now documented rather than smoothed over: Standard QR reports a content overflow as `InvalidOperationException` where Micro QR uses `ArgumentException`, and Standard QR reports an undefined `ECCLevel` as `ArgumentException` where the other two use `ArgumentOutOfRangeException`.
- **Two review rounds were right and one was half right, and telling them apart needed measurement, not judgement.** The claim that the `Try` overload shared the `Get` overload's wrong exception documentation was false, since `Try` does not throw for overflow at all, but the same comment pointed at a genuine gap beside it. Accepting the comment wholesale would have introduced a wrong doc; rejecting it wholesale would have kept a real one. The `Create*` overloads carry the same defect class and were deliberately left out of this phase to keep the diff on its subject; they are tracked separately.
